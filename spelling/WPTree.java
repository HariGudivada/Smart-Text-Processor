...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		//String tocheck = "";
		while(!queue.isEmpty() && !visitedSet.contains(word2)){
			WPTreeNode curr = queue.remove();
			List<String> neighbors = nw.distanceOne(curr.getWord(), true);
			for(String distanceOneWord : neighbors){
				if(!visitedSet.contains(distanceOneWord)){
					curr.addChild(distanceOneWord);
					visitedSet.add(distanceOneWord);
					WPTreeNode randomNode = new WPTreeNode(distanceOneWord, curr);
					queue.add(randomNode);
					if(distanceOneWord.equals(word2)){
						return randomNode.buildPathToRoot();
					}
				}
			}
		}
