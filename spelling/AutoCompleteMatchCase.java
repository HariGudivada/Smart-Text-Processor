...
public class AutoCompleteMatchCase implements  Dictionary, AutoComplete {
	 ...
	 * in the dictionary.
	 */
    private int isUpperCase(String word){
	...
	public boolean addWord(String word)
	{
		...
		int isUpperCase = isUpperCase(word);
		if(isUpperCase==0){
    	 ...
    	 while (!nodeQueue.isEmpty() && count < numCompletions) {
    		 ...
    		 if (tn.endsWord()) {
    			 if(result.isEmpty()){
    				 ...
    				 String lastElement = result.remove(result.size()-1);
    				 if(tn.getText().length()<lastElement.length()){
    					 result.add(tn.getText());
