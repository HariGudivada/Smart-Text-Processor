...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= s.length(); index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				System.out.println("Before Insertion: "+sb.toString()+", index: "+index+", char: "+(char)charCode);
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for (int index = 0; index < s.length(); index++) {
			...
			//if(!previousString.equals(sb.toString())){
...
//		          add n to the list of words to return
		int count = 0;
		int thr  =0 ;
		while(!queue.isEmpty()&& count<numSuggestions && thr < THRESHOLD)
		{
			String curr = queue.remove(0);
			List<String> oneDistanceList = distanceOne(curr, true);
			for(String mutatedString:oneDistanceList){
				if(!visited.contains(mutatedString)){
					visited.add(mutatedString);
					queue.add(mutatedString);
					if(dict.isWord(mutatedString)){
						retList.add(mutatedString);
						count++;
					}
				}
			thr++;	
