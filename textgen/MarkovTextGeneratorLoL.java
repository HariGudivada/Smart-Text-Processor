...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		for(int i = 1 ; i <= source.length ; i++){
			...
			listNode= requiredNode(prevWord);
			if (listNode == null) {
				listNode = new ListNode(prevWord);
	...
	private ListNode requiredNode(String word) {
		for(ListNode listNode: wordList){
			if((word.equals(listNode.getWord()))){
				return listNode;
			}
		}
		return null;
	...
	public String generateText(int numWords) {
		...
		while(count < numWords){
			ListNode requiredNode = requiredNode(currWord);
			Random randomGenerator = new Random();
			String randomWord = requiredNode.getRandomNextWord(randomGenerator);
